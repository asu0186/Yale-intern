...
train_scores$target <- ifelse(train$Condition=="Pre",0,1)
test_scores$target <- ifelse(test$Condition=="Pre",0,1)
train_scores_rot$target <- ifelse(train$Condition=="Pre",0,1)
test_scores_rot$target <- ifelse(test$Condition=="Pre",0,1)
...
probabilities[154]=mean(probabilities)
...
mydata <- mydata %>%
  mutate(mydata,logit = log(probabilities/(1-probabilities))) %>%
...
nn <- neuralnet(target~., train_scores, linear.output = FALSE)
nk <- neuralnet(target~., test_scores, linear.output = FALSE)
print(nn)
print(nk)


library(elasticnet)
obj4 <- cv.enet(nk$covariate,nk$response,lambda=0.05,s=seq(0,1,length=100),mode="fraction",trace=TRUE,max.steps=80)
plot(obj4)
obj4
obj5 <- cv.enet(nn$covariate,nn$response,lambda=0.05,s=seq(0,1,length=100),mode="fraction",trace=TRUE,max.steps=80)
plot(obj4)
plot(obj5)

r=compute(nn,train_scores)
r$net.result

q=compute(nk,test_scores)
q$net.result
